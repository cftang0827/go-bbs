...
func NewBoardHeaderWithByte(data []byte) (*BoardHeader, error) {
	...
	ret.Next = [2]int32{int32(binary.LittleEndian.Uint32(data[PosOfNext : PosOfNext+4])), int32(binary.LittleEndian.Uint32(data[PosOfNext+4 : PosOfNext+8]))}
	ret.FirstChild = [2]int32{int32(binary.LittleEndian.Uint32(data[PosOfFirstChild : PosOfFirstChild+4])), int32(binary.LittleEndian.Uint32(data[PosOfFirstChild+4 : PosOfFirstChild+8]))}
...
func NewBoardFromRaw(boardHeaderRaw *ptttype.BoardHeaderRaw) *BoardHeader {
	boardHeader := &BoardHeader{}

	boardHeader.BrdName = Big5ToUtf8(bytes.Trim(boardHeaderRaw.Brdname[:], "\x00"))
	boardHeader.Title = Big5ToUtf8(bytes.Split(boardHeaderRaw.Title[:], []byte("\x00"))[0]) // Be careful about C-string end char \0
	boardHeader.BM = string(bytes.Trim(boardHeaderRaw.BM[:], "\x00"))
	boardHeader.Brdattr = boardHeaderRaw.BrdAttr
	boardHeader.VoteLimitPosts = uint8(boardHeaderRaw.VoteLimitPosts_)
	boardHeader.VoteLimitLogins = uint8(boardHeaderRaw.VoteLimitLogins)
	boardHeader.ChessCountry = string(bytes.Trim([]byte{boardHeaderRaw.ChessCountry}, "\x00"))
	boardHeader.BUpdate = time.Unix(int64(boardHeaderRaw.BUpdate), 0)
	boardHeader.PostLimitPosts = uint8(boardHeaderRaw.PostLimitPosts_)
	boardHeader.PostLimitLogins = uint8(boardHeaderRaw.PostLimitLogins)
	boardHeader.BVote = uint8(boardHeaderRaw.BVote)
	boardHeader.VTime = time.Unix(int64(boardHeaderRaw.VTime), 0)
	boardHeader.Level = boardHeaderRaw.Level
	boardHeader.PermReload = time.Unix(int64(boardHeaderRaw.PermReload), 0)
	boardHeader.Gid = boardHeaderRaw.Gid

	boardHeader.Next = boardHeaderRaw.Next
	boardHeader.FirstChild = boardHeaderRaw.FirstChild

	boardHeader.Parent = boardHeaderRaw.Parent
	boardHeader.ChildCount = boardHeaderRaw.ChildCount

	boardHeader.Nuser = boardHeaderRaw.NUser
	boardHeader.PostExpire = boardHeaderRaw.PostExpire
	boardHeader.EndGamble = time.Unix(int64(boardHeaderRaw.EndGamble), 0)
	boardHeader.PostType = Big5ToUtf8(bytes.Trim(boardHeaderRaw.PostType[:], "\x00"))
	boardHeader.PostTypeF = Big5ToUtf8([]byte{boardHeaderRaw.PostTypeF})

	boardHeader.FastRecommendPause = boardHeaderRaw.FastRecommendPause
	boardHeader.VoteLimitBadPost = boardHeaderRaw.VoteLimitBadpost
	boardHeader.PostLimitBadPost = boardHeaderRaw.PostLimitBadpost
	boardHeader.SRexpire = time.Unix(int64(boardHeaderRaw.SRexpire), 0)
