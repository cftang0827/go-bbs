...
func TestBoardRaw(t *testing.T) {
	file, err := os.Open("testcase/board/01.BRD")
	a, err := ptttype.NewBoardHeaderRawWithFile(file)
	_ = NewBoardFromRaw(a[0])
	if err != nil {
		t.Error(err)
	}
	for i, v := range a {
		fmt.Println(i, v, unsafe.Sizeof(*v))
	}
}
